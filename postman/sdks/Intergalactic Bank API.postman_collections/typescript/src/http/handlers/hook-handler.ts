...
export class HookHandler implements RequestHandler {
  ...
  async handle<T>(request: Request): Promise<HttpResponse<T>> {
    ...
    const arrayBuffer = await this.extractArrayBuffer(response.raw);
  ...
  /**
   * Extracts the response body as an ArrayBuffer from the raw response.
   * @param raw - The raw response object
   * @returns A promise that resolves to the response body as ArrayBuffer
   */
  private async extractArrayBuffer(raw: Response): Promise<ArrayBuffer> {
